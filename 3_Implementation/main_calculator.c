...
{
	return (strncmp(tk, "abs", 3) == 0 || strncmp(tk, "floor", 5) == 0 || strncmp(tk, "ceil", 4) == 0 || strncmp(tk, "sin", 3) == 0 || strncmp(tk, "cos", 3) == 0 || strncmp(tk, "tan", 3) == 0 || strncmp(tk, "arcsin", 6) == 0 || strncmp(tk, "arccos", 6) == 0 || strncmp(tk, "arctan", 6) == 0 || strncmp(tk, "asin", 4) == 0 || strncmp(tk, "acos", 4) == 0 || strncmp(tk, "atan", 4) == 0 || strncmp(tk, "sqrt", 4) == 0 || strncmp(tk, "cbrt", 4) == 0 || strncmp(tk, "log", 3) == 0 || strncmp(tk, "min", 3) == 0 || strncmp(tk, "max", 3) == 0 || strncmp(tk, "sum", 3) == 0 || strncmp(tk, "avg", 3) == 0 || strncmp(tk, "mean", 4) == 0 || strncmp(tk, "median", 6) == 0 || strncmp(tk, "var", 3) == 0 || strncmp(tk, "exp", 3) == 0);
}
...
int tokenize(char *str, char *(**tokensRef))
{
	...
	while((ch = *ptr++))
	{
		...
		switch(type(ch))
		{
			case addop:
			{
				if(ch == '-' && (numTokens == 0 || (tokenType(tokens[numTokens-1]) == addop || tokenType(tokens[numTokens-1]) == multop	|| tokenType(tokens[numTokens-1]) == expop || tokenType(tokens[numTokens-1]) == lparen || tokenType(tokens[numTokens-1]) == argsep)))
				{
					int len = 1;
					...
					for(;*ptr && len <= prefs.maxtokenlength && (type(*ptr) == digit || ((type(*ptr) == decimal	&& hasDecimal == 0)) || ((*ptr == 'E' || *ptr == 'e') && hasExponent == false) || ((*ptr == '+' || *ptr == '-') && hasExponent == true));++len)
					{
						if(type(*ptr) == decimal)
						{	
							hasDecimal = true;
...
bool postfix(token *tokens, int numTokens, Stack *output)
{
	...
	for(i = 0; i < numTokens; i++)
	{
		switch(tokenType(tokens[i]))
		{
			...
			case expop:
			{
				while(stackSize(&operators) > 0	&& (tokenType((char*)stackTop(&operators)) == addop || tokenType((char*)stackTop(&operators)) == multop || tokenType((char*)stackTop(&operators)) == expop) && ((leftAssoc(tokens[i]) && precedence(tokens[i], (char*)stackTop(&operators)) <= 0) || (!leftAssoc(tokens[i]) && precedence(tokens[i], (char*)stackTop(&operators)) < 0)))
				{
					evalStackPush(output, stackPop(&operators));
					stackPush(&intermediate, stackTop(output));
				}
...
int main(int argc, char *argv[])
{
	...
	int a;
	printf("Press 1 for matrix mode or 0 for others\n");
	scanf("%d",&a);
	if(a==1)
	{
		matx();
